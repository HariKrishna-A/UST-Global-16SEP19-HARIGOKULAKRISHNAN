package com.ustglobal.demo.service;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.ustglobal.demo.dto.EmployeeBean;
import com.ustglobal.demo.repo.EmployeeRepo;
@Service
public class EmployeeJpaIMPL implements EmployeeService{

	@Autowired
	private EmployeeRepo repo;
	
	@Override
	public boolean addEmployee(EmployeeBean bean) {
		repo.save(bean); //return employee object
		return true;
	}

	@Override
	public boolean modifyEmployee(EmployeeBean bean) {
		repo.update(bean.getName(),bean.getPassword(),bean.getDoj(),bean.getGender(),bean.getId());
		return true;
	}

	@Override
	public boolean deleteEmployee(int id) {
//		EmployeeBean bean = repo.findById(id).get();
//		repo.delete(bean);
		repo.deleteById(id);
		return true;
	}

	@Override
	public EmployeeBean getEmployee(int id) {
		
		return repo.findById(id).get();
	}

	@Override
	public List<EmployeeBean> getAllEmployee() {
		return repo.findAll();
	}

}
